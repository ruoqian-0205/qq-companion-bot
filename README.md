...
一个通过 [NapCat](https://github.com/NapNeko/NapCatQQ) 接入 QQ 的**陪伴型**聊天机器人,由本地 Python 脚本驱动。不是关键词应答机 —— 它会记住你说过的事,并有一套自己的性格。

**它和普通 QQ 机器人不一样的地方:**

- **聊得像个真人** —— 私聊可调回复概率,不是每句都抢着回;回复能拆成多条短句,像人那样连发
- **有长期记忆** —— 记得住你是谁、你们之间发生过什么,还慢慢长出自己对这段关系的偏好与心事
- **看群聊的人数说话** —— 冷群主动捧场,热聊时安静避让,不刷屏也不装死
- **能看懂你发的表情** —— 包括「狗头」「菜汪」这类新版 QQ 表情,也会用表情回你

**快速开始:**

```bash
pip install -r requirements.txt
cp .env.example .env                    # 密钥,见「配置密钥」
cp config.example.json config.json      # 配置,见「配置 config.json」
cp persona.example.txt persona.txt      # 人设,见「配置人设」
python bot.py                           # 先跑 python bot.py --debug 可不连 QQ 试人设
```
...
| **私聊长期记忆** | **仅私聊**。三层结构:对话窗口 + 事实库(`他是谁`/`我们之间`/`事件` 三桶) + **心事**(相处中长出的偏好、自我觉察与没说出口的欲望),另有近期流水;不使用向量检索;每个好友独立;支持 `%#` 彻底删除 |
...
> **模型**:默认用 DeepSeek(`deepseek-flash`,开启思考模式)做文字对话、阿里云通义千问(`qwen3.7-flash`)做图片识别。两者都能在 `config.json` 里换成任意 OpenAI 兼容接口。
...
| `clear_memory_reply` | `喵~ 记忆已经清空啦，我们重新开始吧！` | 执行 `%#` 后的回复 |
...
| `inner_clear_memory_reply` | `「」` | 里人格账号的 `%#` 回复;留空则回退到 `bot.clear_memory_reply`,**并在启动时告警** |
...
| `engage_followup_seconds` | `60`           | 「紧跟它发言」的判定窗口 |
...
| 指令 | 效果 |
|---|---|
| `%#` | 彻底清空当前会话:私聊会**同时删除**对话窗口与长期事实库;群聊清空该群上下文 |
| `%n` | 撤回机器人**最近 n 条**消息(如 `%1` 撤一条、`%3` 撤三条),并删掉它们在记忆里的条目 |
| `%%` | 撤回机器人**还撤得掉的**全部消息(不设条数上限) |

> 指令用 `%` 前缀是为了不和日常聊天撞车,并且**要求整条消息只有指令本身**
> (前后空白可以)—— 所以聊到「50%的内容」不会被误判成指令。
...
**它也能自己发。** 写法与对话记录里看到的**完全一样**:`{表情:名字}`,
例如「你说得对{表情:狗头}」。一种写法、零转换 —— 模型看到什么就照着写什么。

- **位置随意** —— 句末、单独发一个、放开头都行。
- **标记两侧的空格会被清掉** —— 模型常写成「行吧 {表情:狗头}」,那个空格渲染出来
  会让表情前多出一格,代码会主动吃掉它(只吃紧贴标记的,不动句子内部的空格)。
- **名字写错会被整个丢掉** —— 认不出的名字映射不到 id,那个标记直接从消息里删掉
  (不发送、不进记忆、也不留 `{表情:瞎编}` 这种字面量)。旧写法 `{比心}` 也认,
  入库时会被规范成 `{表情:比心}`。

用户发的和机器人发的,存进对话记录时是同一种形态:

    他: 看这个{表情:比心}
    我: 看你的{表情:比心}

规范化与过滤都收在 `append_memory`(所有写入记忆的路径都经过它),
所以记忆里不会出现两种写法,也不会有假标记。
...
- **失败会明确说**:「已超出可撤回时限(约 110 秒)」、「服务端拒绝」、「没收到回执」。
  不会出现「发了指令却毫无反应」。这条提示**发出 10 秒后会自动撤回** ——
  它只是操作回执,不该长期占着聊天(时长可调 `bot.recall_tip_ttl`)。
- **失败只报一次,且不会自动往前找**:`%3` 里若有两条撤成功、一条超窗,
  就不算失败(撤掉的那两条你本来就看得见),不会为剩下的那条再挂一条提示。
- **没撤够会说一声**:`%5` 而窗口内只有 2 条时,撤完这 2 条会补一句
  「只有 2 条还撤得掉,已全部撤回」—— 免得你以为自己发过 5 条、
  或者以为指令没生效。这条提示同样 10 秒后自动撤回。
- **`%%` 不设条数上限**,窗口内还撤得掉的会一条条撤完(每条之间隔 0.8 秒防风控)。
...
**Q: 说了 `%#` 以后,机器人还会记得以前的事吗?**

A: 不会。私聊的 `%#` 会**同时**删除对话窗口(`memory.json`)和长期事实库(`memory_long.json`)里该好友的全部内容,并作废进行中的整理任务。
...
- ⚠️ 启用长期记忆后,私聊内容会被**长期留存**,并整理成事实库与「心事」(不再是只留最近几十条)。请确认对方知情且同意,必要时用 `%#` 彻底删除
