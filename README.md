...
| `clear_memory_reply` | `喵~ 记忆已经清空啦，我们重新开始吧！` | 执行 `q清空记忆` 后的回复 |
| `recall_max` | `20` | `y一键撤回` 一次最多撤回几条 |
| `recall_interval` | `0.8` | `y一键撤回` 每条之间的间隔(秒),避免快速连续调用触发风控 |
| `recall_max_try` | `8` | `c撤回` 一次最多往前尝试几条。QQ 的撤回时限是硬的,一旦最新那条已过期就永远撤不掉;只弹一条会被它反复卡住(表现为"发了没反应"),所以往前找 |
| `recall_restore_window_seconds` | `1800` | 重启后从记忆重建撤回登记表时,只认这段时间内发过的消息。更早的必然已过 QQ 撤回时限,恢复它们只会让 `c撤回` 挨个去撞墙 |
...
| `engage_up` | `0.75` | 被 @ / 紧跟它发言时，介入深度升多少 |
...
| 指令 | 效果 |
|---|---|
| `q清空记忆` | 彻底清空当前会话:私聊会**同时删除**对话窗口与长期事实库;群聊清空该群上下文 |
| `c撤回` | 撤回机器人**本会话发过的最后一条还撤得掉的消息**,并删掉它在记忆里的对应条目 |
| `y一键撤回` | 撤回机器人**本会话发过的**消息(最多 `bot.recall_max` 条,默认 20,每条间隔 `bot.recall_interval` 秒),并删掉对应记忆条目 |

两个撤回指令**静默执行**，机器人不会回话。

**为什么 `c撤回` 会往前找而不是只撤最新一条**：QQ 侧的撤回时限（约 2 分钟）是硬的，一旦最新那条已经过期，它就永远撤不掉。如果实现成"只弹一条"，这一条会**反复卡住**每一次 `c撤回`——一条消息都不消失，看起来就是"发了完全没反应"。所以它从最新往回找，`recall_max_try` 条以内找到第一条真的撤得掉的为止；中间那些撤不掉的顺手出栈（它们本来也永远撤不掉，不该再挡路），但**不删它们的记忆**——消息还在聊天里，记忆就该留着。`y一键撤回` 因为一次扫一批、撤不掉的跳过，所以同样的时限下它看起来一切正常。

**机器人也能感知别人撤回消息**：一旦收到撤回通知，会把对应的记忆条目一并删掉。为此 `memory.json` 里每条消息都记了它在 QQ 侧的 `message_id`（旧记录没有这个字段，不影响使用，只是那部分无法被撤回同步）。撤回**只影响 L0**——L1 里的内容是被模型压缩改写过的间接表述，已经无法对应回某条具体消息。

被撤回的如果**正好是机器人自己发的**（你手动撤回、或群管理撤回），它还会把这条从撤回登记表里划掉——否则那条记录永远撤不掉了，却会在下次 `c撤回` 时被弹出来挡路。

重启后登记表会按 `bot.recall_restore_window_seconds` 从记忆里重建（默认 30 分钟），所以刚发完消息就重启，`c撤回` 依然能用；更早的记录不再恢复——它们已经过了 QQ 的撤回时限，恢复也只是摆设。
...
**Q: 说了 `q清空记忆` 以后,机器人还会记得以前的事吗?**

A: 不会。私聊的 `q清空记忆` 会**同时**删除对话窗口(`memory.json`)和长期事实库(`memory_long.json`)里该好友的全部内容,并作废进行中的整理任务。
