...
- **近期流水单独一层**:L0 只覆盖最近几小时(实测高密度对话约 100 条/小时,500 条也就 5 小时),而事实库只记长期属性,中间"最近聊过些什么"没有归宿——模型于是对昨天的事一无所知。`recent` 补的就是这一段
...
| `recent` | 最近的日常流水 | **增量追加** | 保留最近 3 个聊过的日子,代码按记录日整天淘汰;超过 200 条从最老的一天删 |
